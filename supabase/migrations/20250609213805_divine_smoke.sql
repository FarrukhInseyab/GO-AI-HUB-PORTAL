/*
  ...
  2. Security
    ...
    - Grant appropriate access to different roles
*/
...
-- Drop any RLS-related indexes that are no longer needed (with error handling)
DO $$
BEGIN
  DROP INDEX IF EXISTS idx_users_user_id_role;
  DROP INDEX IF EXISTS idx_solutions_approval_status;
  RAISE NOTICE 'Dropped RLS-related indexes';
EXCEPTION
  WHEN OTHERS THEN
...
DO $$
BEGIN
  -- Grant permissions only to tables that exist
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
...
BEGIN
  ...
  RAISE NOTICE 'Granted read permissions to anonymous users';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Profiles table was skipped as it does not exist';
END $$;